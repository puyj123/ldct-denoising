...
parser.add_argument('--input_dir', default='C:/科研/SUNET/SUNet-main/SUNet-main/datasets/', type=str, help='Input images')
...
npy_file_path = '.\L506_0_input.npy'
# 使用NumPy的load函数加载.npy文件
data = np.load(os.path.join(npy_file_path))
print(data)
...
def overlapped_square(timg, kernel=256, stride=128):
    ...
    img = torch.zeros(1, 1, X, X).type_as(timg)  #add
...
#data_result=overlapped_square(data)  #add
...
for file_ in files:
    ...
    with torch.no_grad():
        ...
        square_input_, mask, max_wh = overlapped_square(data) #add
